...
void siv() {
	P[0] = P[1] = 1;
	for(int i = 4 ; i <= Max ; i += 2) P[i] = 1;
	for(int i = 3 ; i * i <= Max ; i += 2) {
		if(!P[i]) {
			for(int j = i * i , k = i << 1 ; j <= Max ; j += k) P[j] = 1;
		}
	}
	FOR(i , Max) P[i] ^= 1;
}
...
void build(int lo = 1 , int hi = N , int node = 1) {
	if(lo == hi) {tree[node] = P[ arr[lo] ], lazy[node] = 0; return ;}
