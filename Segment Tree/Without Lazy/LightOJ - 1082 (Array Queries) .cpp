...
void build(int lo = 1, int hi = N , int node = 1) {
	/* Do U really Build */
	/* 1 Based. Be Careful */
	/* tree size is 4 size multiple of MAXN */
	if(lo == hi) { tree[node] = arr[lo] ; return; }
	mlr ; build(lo , mid , l) , build(mid + 1 , hi , r);
	tree[node] = min(tree[l], tree[r]);
}

int get(int x , int y , int lo = 1, int hi = N , int node = 1){
	if(lo > hi || lo > y || hi < x) return INT_MAX;
	if(x <= lo && hi <= y) return tree[node]; mlr ;
	return min(get(x , y , lo , mid , l) , get(x , y , mid + 1 , hi, r));
}
