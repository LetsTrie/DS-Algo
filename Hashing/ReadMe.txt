...
void ForHash() {
    ...
    for(int i = 1 ; i <= lent ; i++) {
        ...
        H[1][i] = (H[1][i - 1] + (text[i - 1] * B[1][i]) % M[1] ) % M[1];  
    }
...
void RevHash() {
    ...
    for(int i = 1; i <= lent ; i++) {
        ...
        RH[1][i] = (RH[1][i - 1] + (text[i - 1] * B[1][lent - i + 1]) % M[1] ) % M[1];  
    }
}


pair<LL,LL> i2jRev(int i , int j) {
...
int solve() {
    ...
    for(int i = 0; i + lenp - 1 < lent ; i++) {
        pair<LL,LL> value = i2jFor(i , i + lenp - 1);
        if(value.fi == pH.fi && value.se == pH.se) {
        	cnt++;
        }
    }
    return cnt;
